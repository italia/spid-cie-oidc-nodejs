...
import { createAuditLogRotatingFilesystem } from "../../src";

describe("audit log rotating file system", () => {
  test("it should write to filesystem", async () => {
    await new Promise((resolve) => rimraf("logs/audit*", resolve));
    const logger = createAuditLogRotatingFilesystem();
    const now = new Date();
    logger("test");
    ...
    const filename = `logs/audit-${year}-${month}-${date}-${hours}.log`;
